            ...
            public function eliminar_cliente_prestamo_controlador(){
                ...
                if(empty($_SESSION['datos_cliente'])){
                    ...
                    $alerta=[
                        "Alerta"=>"simple",
                        "Titulo"=>"Ocurrió un error inesperado",
                        "Texto"=>"No hemos podido remover el cliente ",
                        "Tipo"=>"error"
                    ];
    ...
    public function eliminar_item_prestamo_controlador(){ 
     ...
     public function datos_prestamo_controlador($tipo,$id){ 
        ...
        return prestamoModelo::datos_prestamo_modelo($tipo,$id);
         ...
         //controlador registrar prestamos
    public function agregar_prestamo_controlador(){ 
        ...
        if (empty($_SESSION['datos_cliente'])) {
         ...
         /*== Verificando integridad de los datos ==*/
            ...
            //Formateando totales, fecha y hora
    ...
    if ($tipo=="Busqueda") {
       if (mainModel::verificar_fecha($fecha_inicio) || mainModel::verificar_fecha($fecha_final)) {
        return '
            ...
            <p class="mb-0">Lo sentimos, no podemos realizar la busquedad porque ha ingresado una fecha que no es correcta.</p>
            </div>
        ';
       }
    }

    $campos="prestamo.prestamo_id,prestamo.prestamo_codigo,prestamo.prestamo_fecha_inicio,prestamo.prestamo_fecha_final,prestamo.prestamo_total,prestamo.prestamo_pagado,
    prestamo.prestamo_estado,prestamo.usuario_id,prestamo.cliente_id,cliente.cliente_nombre,cliente.cliente_apellido";
	...
	if($total>=1 && $pagina<=$Npaginas){
		...
		foreach($datos as $rows){
			...
			<tr class="text-center" >
                ...
                $tabla.='
                    <td>
                    <a href="'.SERVERURL.'facturas/invoice.php?id='.mainModel::encryption($rows['prestamo_id']).'" class="btn btn-info"
                    target="_blank">
                ...
                ';
						...
						if($privilegio==1 || $privilegio==2){
                            ...
                            if ($rows['prestamo_estado']=="Finalizado" && $rows['prestamo_pagado']==$rows['prestamo_total']) {
                                ...
                                $tabla.='<td>
							    <a href="'.SERVERURL.'reservation-update/'.mainModel::
							    encryption($rows['prestamo_id']).'" class="btn 
							    btn-success">
									<i class="fas fa-sync-alt"></i>	
							    </a>
						        </td>';
