...
class itemModelo extends mainModel {
 ...
 protected static function eliminar_item_modelo($id){
  ...
  /*__Modelo edatos item__*/
  protected static function datos_item_modelo($tipo, $id){
		if($tipo=="Unico"){
			$sql=mainModel::conectar()->prepare("SELECT * FROM item WHERE
			 item_id =:ID");
			 $sql->bindParam(":ID",$id);
		}elseif($tipo=="Conteo"){
      ...
      /*__Modelo actualizar item__*/
		protected static function actualizar_item_modelo($datos){
			$sql=mainModel::conectar()->prepare("UPDATE item SET item_codigo =:Codigo,item_nombre=:Nombre, item_stock=
			:Stock,item_estado=:Estado, item_detalle=:Detalle WHERE item_id=:ID");
